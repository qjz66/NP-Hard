...
namespace AutoBenchmark {
    ...
    public class Checker {
        ...
        public static void latinSquare(string[] input, string output, Statistic statistic) {
                ...
                if (words.Length == n * n) {
                    ...
                    for (int i = 0; i < n; ++i) {
                        for (int j = 0; j < n; ++j) {
                            ...
                            if ((nodeColors[i, j] < 0) || (nodeColors[i, j] >= n)) { ++numOutOfRange; }
                            if ((fixedColors[i, j] != -1 ) && (fixedColors[i, j] != nodeColors[i, j])) { ++fixedNumChanged; }
                        }
                    }
                }
            } catch (Exception e) { Util.log("[error] checker load output fail due to " + e.ToString()); }
            ...
            int conflictNum = 0;
            try { // check.
                if (sizeMatch && (numOutOfRange == 0) && (fixedNumChanged == 0)) {
                    for (int i = 0; i < n; ++i) {
                        bool[] numExists = Enumerable.Repeat(false, n).ToArray();
                        for (int j = 0; j < n; ++j) {
                            if (numExists[nodeColors[i, j]]) { ++conflictNum; break; }
                            numExists[nodeColors[i, j]] = true;
                        }

                        numExists = Enumerable.Repeat(false, n).ToArray();
                        for (int j = 0; j < n; ++j) {
                            if (numExists[nodeColors[j, i]]) { ++conflictNum; break; }
                            numExists[nodeColors[j, i]] = true;
                        }
                    }
                }
            } catch (Exception e) { Util.log("[error] checker check fail due to " + e.ToString()); }

            bool feasible = sizeMatch && (numOutOfRange == 0) && (fixedNumChanged == 0);
            statistic.obj = feasible ? conflictNum : Problem.MaxObjValue;
            statistic.info = (sizeMatch ? 1 : 0).ToString() + BenchmarkCfg.LogDelim + numOutOfRange.ToString() + BenchmarkCfg.LogDelim + fixedNumChanged.ToString();
        }
    }
}
