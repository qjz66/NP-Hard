...
public:
	void solve(Nodes& output, MinCDominatingSet& input, function<long long()> restMilliSec, int seed) {
		initRand(seed);
		...
		//                      +----[ exit before timeout ]
		//                      |
		for (NodeId n = 0; (restMilliSec() > 0) && (n < 100); ++n) { output[n] = rand(input.nodeNum); }
		//                                                                        |
...
// solver.
void solveMinCDominatingSet(Nodes& output, MinCDominatingSet& input, function<long long()> restMilliSec, int seed) {
