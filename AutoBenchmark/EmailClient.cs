...
namespace AutoBenchmark {
    public class EmailFetcher {
        ...
        static bool handleMessageBuildCpp(MimeMessage msg) {
            DateTime now = DateTime.Now;
            ...
            try {
                ...
                foreach (var file in msg.Attachments) {
                    string fileName = file.ContentDisposition?.FileName;
                    if (fileName == null) { continue; }
                    if (CommonCfg.ZipFileExts.Contains(Path.GetExtension(fileName))) {
                        using (MemoryStream ms = ((MimePart)file).toStream())
                        using (ArchiveFile archiveFile = new ArchiveFile(ms)) {
                            foreach (Entry entry in archiveFile.Entries) {
                                if (entry.Size > EmailCfg.MaxFileByteSize) { Util.log($"[warning] skip file larger than {EmailCfg.MaxFileByteSize}B"); continue; }
                                if (entry.FileName.Contains("__MACOSX") || entry.FileName.Contains(".DS_Store")) { continue; }
                                string path = Path.Combine(dirPath, entry.FileName);
                                entry.Extract(path);

                                string ext = Path.GetExtension(path);
                                if (CommonCfg.srcFileExts.Contains(ext)) { srcFiles.Add(path); }
                            }
                        }
                    } else {
                        using (FileStream fs = File.Create(fileName)) { ((MimePart)file).Content.DecodeTo(fs); }
                    }
                }
            } catch (Exception e) {
                Util.log("[error] save attachment fail due to " + e.Message);
            ...
            s.exePath = Path.Combine(dirPath, CommonCfg.ExeName);
            string buildLog = CppBuilder.cl(dirPath, srcFiles, s.exePath);
            ...
            Util.log("[error] compiler or linker error.");
            StdSmtp.send(s.email, "Re: " + msg.Subject, buildLog.Replace("\n", "<br />"));
            return false;
        }
    }
